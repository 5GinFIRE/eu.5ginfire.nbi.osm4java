...
public class VduConfigurationBuilder implements Builder<VduConfiguration> {
	...
	Map<Class<? extends Augmentation<VduConfiguration>>, Augmentation<VduConfiguration>> augmentation = Collections
			.emptyMap();
	...
	public VduConfigurationBuilder(
			ns.yang.nfvo.mano.types.rev170208.VcaConfiguration arg) {
	...
	public VduConfigurationBuilder(
			ns.yang.nfvo.mano.types.rev170208.VcaMetrics arg) {
		this._metrics = arg.getMetrics();
	}
	...
	public void fieldsFrom(DataObject arg) {
		...
		if (arg instanceof ns.yang.nfvo.mano.types.rev170208.VcaConfiguration) {
			this._configMethod = ((ns.yang.nfvo.mano.types.rev170208.VcaConfiguration) arg)
					.getConfigMethod();
			this._configPrimitive = ((ns.yang.nfvo.mano.types.rev170208.VcaConfiguration) arg)
					.getConfigPrimitive();
			this._initialConfigPrimitive = ((ns.yang.nfvo.mano.types.rev170208.VcaConfiguration) arg)
					.getInitialConfigPrimitive();
			isValidArg = true;
		}
		if (arg instanceof ns.yang.nfvo.mano.types.rev170208.VcaMetrics) {
			this._metrics = ((ns.yang.nfvo.mano.types.rev170208.VcaMetrics) arg)
					.getMetrics();
			isValidArg = true;
		}
		CodeHelpers.validValue(isValidArg, arg,
				"[model.urn.ietf.params.xml.ns.yang.nfvo.mano.types.rev170208.VcaConfiguration, model.urn.ietf.params.xml.ns.yang.nfvo.mano.types.rev170208.VcaMetrics]");
	}
	...
	public VduConfigurationBuilder addAugmentation(Class<? extends Augmentation<VduConfiguration>> augmentationType,
			Augmentation<VduConfiguration> augmentationValue) {
	...
	public VduConfigurationBuilder removeAugmentation(
			Class<? extends Augmentation<VduConfiguration>> augmentationType) {
		if (this.augmentation instanceof HashMap) {
	...
	private static final class VduConfigurationImpl implements VduConfiguration {
		...
		private Map<Class<? extends Augmentation<VduConfiguration>>, Augmentation<VduConfiguration>> augmentation = Collections
				.emptyMap();
		...
		public boolean equals(Object obj) {
			...
			if (getClass() == obj.getClass()) {
				...
				for (Map.Entry<Class<? extends Augmentation<VduConfiguration>>, Augmentation<VduConfiguration>> e : augmentation
						.entrySet()) {
